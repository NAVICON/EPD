...
public class MainFrame extends MainFrameCommon implements IMapFrame {
	...
	private void initGUI() {
		...
		this.chartPanel
				.setHistoryListener(new HistoryListener(this.chartPanel));
		this.chartPanel.getMap().addProjectionListener(
				this.chartPanel.getHistoryListener());
	...
	public Action getAboutAction() {
		Action aboutEpdShip = new AbstractAction("About EPD-ship",
				new ImageIcon(EPD.getInstance().getAppIcon(16))) {

			private static final long serialVersionUID = 1L;

			@Override
			public void actionPerformed(ActionEvent e) {
				final ImageIcon icon = new ImageIcon(EPD.getInstance()
						.getAppIcon(45));
				...
				aboutText
						.append("The user manual is available from e-navigation.net\n\n");
				...
				aboutText.append("Build date: " + VersionInfo.getBuildDate()
						+ "\n");
				aboutText.append("Home path: "
						+ EPD.getInstance().getHomePath());

				JOptionPane.showMessageDialog(MainFrame.this,
						aboutText.toString(), "About the EPD-ship",
	...
	public List<DockableComponentPanel> getDockableComponentPanels() {
		return Arrays.asList((DockableComponentPanel) chartPanel, scalePanel,
				ownShipPanel, gpsPanel, cursorPanel, activeWaypointPanel,
				aisComponentPanel, nogoPanel, sarPanel, msPntComponentPanel,
				stccComponentPanel, safeHavenPanel);
	}

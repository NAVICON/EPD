...
public final class EPDShip extends EPD implements IOwnShipListener {
	...
	public EPDShip(MapHandler mh) {
		...
		/*
		 * // start riskHandler startRiskHandler();
		 * 
		 ...
		 * 
		 * // Start position handler and add to bean context pntHandler = new
		 * PntHandler(); mapHandler.add(pntHandler);
		 * 
		 ...
		 * 
		 * // Start AIS target monitoring aisHandler = new
		 * AisHandler(settings.getSensorSettings(), settings.getAisSettings());
		 * aisHandler.loadView(); EPD.startThread(aisHandler, "AisHandler");
		 * mapHandler.add(aisHandler);
		 * 
		 * // Start own-ship handler ownShipHandler = new
		 * OwnShipHandler(settings.getAisSettings()); ownShipHandler.loadView();
		 * ownShipHandler.addListener(this); mapHandler.add(ownShipHandler);
		 * 
		 * // Start dynamic predictor handler dynamicPredictorHandler = new
		 * DynamicPredictorHandler(); mapHandler.add(dynamicPredictorHandler);
		 * 
		 ...
		 * 
		 * // Maybe start dynamic prediction generator if
		 * (settings.getSensorSettings().isStartPredictionGenerator()) {
		 * dynamicPredictor = new DynamicPredictor();
		 * mapHandler.add(dynamicPredictor); }
		 * 
		 * 
		 * falManager = FALManager.loadFALManager(); mapHandler.add(falManager);
		 */
		...
		mainFrame = null;
		SARComponentPanel sarPanel = new SARComponentPanel();
		mapHandler.add(sarPanel);
		mapHandler.add(sarPanel.getSARPanel());

		voctManager = VOCTManager.loadVOCTManager();
		mapHandler.add(voctManager);
		// Create VOCT Layer
		VoctLayer voctLayer = new VoctLayer();
		...
		/*
		 * // Create shore services shoreServices = new
		 * ShoreServices(getSettings().getEnavSettings());
		 * mapHandler.add(shoreServices);
		 * 
		 * // Create mona lisa route exchange monaLisaRouteExchange = new
		 * MonaLisaRouteOptimization(); mapHandler.add(monaLisaRouteExchange);
		 * 
		 * // Create MSI handler msiHandler = new
		 * MsiHandler(getSettings().getEnavSettings());
		 * mapHandler.add(msiHandler);
		 * 
		 * // Create NoGo handler nogoHandler = new
		 * NogoHandler(getSettings().getEnavSettings());
		 ...
		 * 
		 * // Create intended route handler intendedRouteHandler = new
		 * IntendedRouteHandler();
		 * intendedRouteHandler.updateSettings(settings.getEnavSettings());
		 * mapHandler.add(intendedRouteHandler);
		 * 
		 * // Create the route suggestion handler // routeSuggestionHandler =
		 * new RouteSuggestionHandler(); routeSuggestionHandler =
		 * RouteSuggestionHandler.loadRouteSuggestionHandler();
		...
		/*
		 * // Create voyage event dispatcher voyageEventDispatcher = new
		 * VoyageEventDispatcher();
		 * 
		 * // Create identity handler identityHandler = new IdentityHandler();
		 * mapHandler.add(identityHandler);
		 */

		/*
		 * // Create FAL Handler falHandler = new FALHandler();
		 * mapHandler.add(falHandler);
		 * 
		 * // Start sensors startSensors();
		 * 
		 * // Create plugin components createPluginComponents();
		 * 
		 * final CountDownLatch guiCreated = new CountDownLatch(1);
		 * 
		 * // Create and show GUI SwingUtilities.invokeLater(new Runnable() {
		 * 
		 * @Override public void run() { createAndShowGUI();
		 * guiCreated.countDown(); } });
		 * 
		 * // Wait for gui to be created try { guiCreated.await(); } catch
		 * (InterruptedException e) {
		 * LOG.error("Interrupted while waiting for GUI to be created", e); }
		 * 
		 * // Create embedded transponder frame /* transponderFrame = new
		 * TransponderFrame(getHomePath().resolve("transponder.xml").toString(),
		 * true, mainFrame); mapHandler.add(transponderFrame);
		 * 
	...
	protected void startSensors() {
		...
		case TCP:
			gpsSensor = new NmeaTcpSensor(
					...
					sensorSettings.getGpsTcpOrUdpPort());
			break;
		case UDP:
			gpsSensor = new NmeaUdpSensor(sensorSettings.getGpsTcpOrUdpPort());
			break;
		case SERIAL:
			gpsSensor = NmeaSerialSensorFactory.create(
					sensorSettings.getGpsHostOrSerialPort(),
		...
		default:
			LOG.error("Unknown sensor connection type: "
					+ sensorSettings.getGpsConnectionType());
		}
		...
		case NONE:
			...
			break;
		case TCP:
			msPntSensor = new NmeaTcpSensor(
					sensorSettings.getMsPntHostOrSerialPort(),
					sensorSettings.getMsPntTcpOrUdpPort());
			break;
		case UDP:
			msPntSensor = new NmeaUdpSensor(
					sensorSettings.getMsPntTcpOrUdpPort());
			break;
		case SERIAL:
			msPntSensor = NmeaSerialSensorFactory.create(
					sensorSettings.getMsPntHostOrSerialPort(),
					sensorSettings.getMsPntSerialPortBaudRate());
			break;
		case FILE:
			msPntSensor = new NmeaFileSensor(sensorSettings.getMsPntFilename(),
					sensorSettings);
			break;
		default:
			LOG.error("Unknown sensor connection type: "
					+ sensorSettings.getMsPntConnectionType());
		}
		...
		if (getSettings().getNavSettings().isDynamicPrediction()) {
			...
			case TCP:
				dynamicPredictorSensor = new NmeaTcpSensor(
						sensorSettings.getDynamicPredictorHostOrSerialPort(),
						sensorSettings.getDynamicPredictorTcpOrUdpPort());
				break;
			case UDP:
				dynamicPredictorSensor = new NmeaUdpSensor(
						sensorSettings.getDynamicPredictorTcpOrUdpPort());
				break;
			case SERIAL:
				dynamicPredictorSensor = new NmeaSerialSensor(
						sensorSettings.getDynamicPredictorHostOrSerialPort(),
		...
		if (dynamicPredictorSensor != null) {
			dynamicPredictorSensor
					.addDynamicPredictorListener(dynamicPredictorParser);
	...
	private void makeKeyBindings() {
		/*
		 * JPanel content = (JPanel) mainFrame.getContentPane(); InputMap
		 * inputMap = content .getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW);
		 * 
		 * @SuppressWarnings("serial") Action zoomIn = new AbstractAction() {
		 * 
		 * @Override public void actionPerformed(ActionEvent actionEvent) {
		 * mainFrame.getChartPanel().doZoom(0.5f); } };
		 * 
		 * @SuppressWarnings("serial") Action zoomOut = new AbstractAction() {
		 * 
		 * @Override public void actionPerformed(ActionEvent actionEvent) {
		 * mainFrame.getChartPanel().doZoom(2f); } };
		 * 
		 * @SuppressWarnings("serial") Action centreOnShip = new
		 * AbstractAction() {
		 * 
		 * @Override public void actionPerformed(ActionEvent actionEvent) {
		 * mainFrame.saveCentreOnShip(); } };
		 * 
		 * @SuppressWarnings("serial") Action newRoute = new AbstractAction() {
		 * 
		 * @Override public void actionPerformed(ActionEvent actionEvent) { //
		 * newRouteBtn.requestFocusInWindow();
		 * mainFrame.getTopPanel().activateNewRouteButton(); } };
		 * 
		 * @SuppressWarnings("serial") Action routes = new AbstractAction() {
		 * 
		 * @Override public void actionPerformed(ActionEvent actionEvent) {
		 * RouteManagerDialog routeManagerDialog = new RouteManagerDialog(
		 * mainFrame); routeManagerDialog.setVisible(true); } };
		 * 
		 * @SuppressWarnings("serial") Action ais = new AbstractAction() {
		 * 
		 * @Override public void actionPerformed(ActionEvent actionEvent) {
		 * mainFrame.getTopPanel().getAisDialog().setVisible(true); } };
		 * 
		 * @SuppressWarnings("serial") Action panUp = new AbstractAction() {
		 * 
		 * @Override public void actionPerformed(ActionEvent actionEvent) {
		 * mainFrame.getChartPanel().pan(1); } };
		 * 
		 * @SuppressWarnings("serial") Action panDown = new AbstractAction() {
		 * 
		 * @Override public void actionPerformed(ActionEvent actionEvent) {
		 * mainFrame.getChartPanel().pan(2); } };
		 * 
		 * @SuppressWarnings("serial") Action panLeft = new AbstractAction() {
		 * 
		 * @Override public void actionPerformed(ActionEvent actionEvent) {
		 * mainFrame.getChartPanel().pan(3); } };
		 * 
		 * @SuppressWarnings("serial") Action panRight = new AbstractAction() {
		 * 
		 * @Override public void actionPerformed(ActionEvent actionEvent) {
		 * mainFrame.getChartPanel().pan(4); } };
		 * 
		 * inputMap.put(KeyStroke.getKeyStroke(java.awt.event.KeyEvent.VK_ADD,
		 * 0), "ZoomIn"); inputMap.put(
		 ...
		 * "panRight"); inputMap.put(KeyStroke.getKeyStroke("control N"),
		 * "newRoute"); inputMap.put(KeyStroke.getKeyStroke("control R"),
		 * "routes"); inputMap.put(KeyStroke.getKeyStroke("control M"), "msi");
	...
	/**
	 * Returns a {@linkplain Resource} instance which loads resource from the
	 * same class-loader/jar-file as the {@code EPDShip} class.
	 * 
	...
	public void ownShipChanged(VesselTarget oldValue, VesselTarget newValue) {
		...
		if (getRouteManager().isRouteActive()) {
			intendedRouteHandler.broadcastIntendedRoute(getRouteManager()
					.getActiveRoute(), true);
		}
	}
}

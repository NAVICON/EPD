 ...
 */
public class CommonBottomPanelStatusDialog extends JDialog implements
		ActionListener {
	...
	/**
	 * <p>
	 * Creates a CommonBottomPanelStatusDialog.<br>
	 * This will create the window, a panel for status and a panel at the bottom
	 * of the window with a "close" button.
	 * 
	...
	/**
	 * Creates the status panels. If any additional panels were given it creates
	 * adds those too.
	 * 
	...
	protected void createStatusPanels(List<JPanel> additionalStatusPanels) {
		...
		aisPanel.setBorder(new TitledBorder(null, "AIS", TitledBorder.LEADING,
		...
		shoreServicesPanel.setBorder(new TitledBorder(null, "Shore Services",
				TitledBorder.LEADING, TitledBorder.TOP, TITLE_FONT));
		...
		maritimeStatuspanel.setBorder(new TitledBorder(null, "Maritime Cloud",
				TitledBorder.LEADING, TitledBorder.TOP, TITLE_FONT));
	...
	protected void showStatus() {
		...
		for (IStatusComponent iStatusComponent : this.statusComponents) {
			...
			if (componentStatus instanceof AisStatus) {
				...
				this.lblAisReceptionStatus.setText(aisStatus.getReceiveStatus()
						.toString());
				this.lblAisSendingStatus.setText(aisStatus.getSendStatus()
						.toString());
				this.lblAisLastReceptionStatus.setText(Formatter
						.formatLongDateTime(aisStatus.getLastReceived()));
				this.lblAisLastSendStatus.setText(Formatter
						.formatLongDateTime(aisStatus.getLastSent()));
				...
				this.lblContactStatus.setText(shoreServices.getStatus()
						.toString());
				this.lblLastContactStatus.setText(Formatter
						.formatLongDateTime(shoreServices.getLastContact()));
				...
				this.lblReceptionStatus.setText(cloudStatus.getStatus()
						.toString());
				this.lblSendingStatus.setText(cloudStatus.getSendStatus()
						.toString());
				this.lblLastReceptionStatus.setText(Formatter
						.formatLongDateTime(cloudStatus.getLastReceived()));
				this.lblLastSendStatus.setText(Formatter
						.formatLongDateTime(cloudStatus.getLastSent()));
	...
	/**
	 * Adds a component to the status frame, and resizes the window height add
	...
	/**
	 * Handles an action event. This is called every 500 ms by the timer object

...
/**
 * Abstract super class for the main system, i.e either {@code EPDShore} or
 * {@code EPDShip}
 */
public abstract class EPD implements ISettingsListener {
	 ...
	 * same class-loader/jar-file as the {@code EPD} class.
	 * 
	...
	/**
	 * Returns the path to the home folder, used for settings, persisted data,
	 * etc.
	 * 
	...
	/**
	 * Starts the sensors as defined in the {@linkplain SensorSettings} and hook
	 * up listeners
	 */
	...
	/**
	 * Stop {@code sensor} and wait at most {@code timeout} ms for it to
	 * terminate.
	 * 
	 * @param sensor
	 *            the sensor to stop
	...
	/**
	 * Informs the user that EPD is already running. Prompts whether to restart
	 * or bail
	 */
	...
	public Image getAppIcon() {
		ImageIcon icon = Resources.get(getInstance().getClass())
				.getCachedImageIcon("/images/appicon.png");
		if (icon != null) {
	...
	public Image getAppIcon(int size) {
		...
		if (appIcon != null) {
			return appIcon.getScaledInstance(size, size,
	...
	/**
	 * If Key caps is pressed during start up, and if so, asks the user for a
	 * home path.<br>

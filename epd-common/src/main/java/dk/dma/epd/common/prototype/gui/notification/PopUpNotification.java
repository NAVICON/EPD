...
/**
 * Defines a pop-up notification panel which displays a message for a certain
 * amount of time.
 * <p>
 * Position-wise, the widget is locked to the bottom-left corner of it's parent
 * layered pane.
 */
public class PopUpNotification extends JPanel implements ActionListener,
		SwingConstants {
	 ...
	 * Constructor
	 * <p>
	 * Installs the widget in the layered pane.
	 * <p>
	 * Position-wise, the widget is locked to the bottom-left corner of it's
	 * parent layered pane.
	 * 
	 * @param layeredPane
	 *            the layered pane
	...
	public PopUpNotification(JLayeredPane layeredPane, Point blLocation) {
		...
		SwingUtilities.getWindowAncestor(layeredPane).addComponentListener(
				resizeHandler);
		...
		notificationList.setLayout(new BoxLayout(notificationList,
				BoxLayout.Y_AXIS));
	...
	/**
	 * Adjusts the size of the pop-up panel to accommodate between 1 and three
	 * notification panels.
	 */
	...
	public synchronized void addNotification(NotificationPanel<?> panel,
		...
		NotificationPopUpPanel notificationPanel = new NotificationPopUpPanel(
				panel, notification);
		notificationPanel.setBorder(BorderFactory.createMatteBorder(0, 0, 2, 0,
				BG_COLOR));
		...
		for (int x = 1; x < notificationList.getComponents().length; x++) {
			NotificationPopUpPanel<?> np = (NotificationPopUpPanel<?>) notificationList
					.getComponents()[x];
			if (np.getNotification().getType().equals(notification.getType())
					&& np.getNotification().getId()
							.equals(notification.getId())
	 ...
	 * <p>
	 * This is the case if, say, the notifications have been acknowledged or
	 * deleted from the notification center.
	 * <p>
	 * This method should thus be called whenever the changes have happened in
	...
	/**
	 * Shows the pop-up. If it has started to fade out, make it fully visible
	 * again.
	 */
	 ...
	 *            the initial delay before it starts closing
	 */
	...
	public static void main(String[] args) {
		...
		PopUpNotification notif = new PopUpNotification(frame.getLayeredPane(),
				location);
	...
	/**
	 * Handles re-positioning of the widget, when the window is resized.
	 * <p>
...
/**
 * The notification pop-up panel displays the title and description for a
 * notification.
 * <p>
 * It also sports buttons for showing the notification in the notification
 * center and for dismissing, i.e. acknowledging, the notification
 */
class NotificationPopUpPanel<N extends Notification<?, ?>> extends JPanel
	...
	public NotificationPopUpPanel(NotificationPanel<N> panel, N notification) {
		...
		add(icon, new GridBagConstraints(0, 0, 1, 2, 0.0, 0.0, CENTER, NONE,
				insets1, 0, 0));

		JLabel titleLbl = new JLabel(notification.getTitle());
		titleLbl.setFont(titleLbl.getFont().deriveFont(11f)
				.deriveFont(Font.BOLD));
		add(titleLbl, new GridBagConstraints(1, 0, 1, 1, 1.0, 0.0, WEST,
				HORIZONTAL, insets1, 0, 0));

		String descHtlm = String.format("<html>%s</html>",
				Formatter.formatHtml(notification.getDescription()));
		JLabel descLbl = new JLabel(descHtlm);
		descLbl.setFont(descLbl.getFont().deriveFont(9f));
		descLbl.setVerticalAlignment(JLabel.TOP);
		add(descLbl, new GridBagConstraints(1, 1, 1, 1, 1.0, 1.0, NORTHWEST,
				BOTH, insets1, 0, 0));

		JPanel btnPanel = new JPanel(new GridBagLayout());
		btnPanel.setOpaque(false);
		add(btnPanel, new GridBagConstraints(2, 0, 1, 2, 0.0, 0.0, NORTHWEST,
				HORIZONTAL, insets1, 0, 0));

		showBtn = createButton("Show");
		btnPanel.add(showBtn, new GridBagConstraints(0, 0, 1, 1, 0.0, 0.0,
				NORTHWEST, HORIZONTAL, insets1, 0, 0));

		if (notification.canAcknowledge()) {
			dismissBtn = createButton("Dismiss");
			btnPanel.add(dismissBtn, new GridBagConstraints(0, 1, 1, 1, 0.0,
					0.0, NORTHWEST, HORIZONTAL, insets1, 0, 0));
		}

		if (notification.getLocation() != null) {
			gotoBtn = createButton("Goto");
			btnPanel.add(gotoBtn, new GridBagConstraints(0, 2, 1, 1, 0.0, 0.0,
					NORTHWEST, HORIZONTAL, insets1, 0, 0));
		}
	}
	...
	public void actionPerformed(ActionEvent ae) {
		if (ae.getSource() == showBtn) {
			EPD.getInstance()
					...
					.openNotification(notification.getType(),
			...
			if (notification.getLocation() != null) {
				EPD.getInstance().getMainFrame()
						.zoomToPosition(notification.getLocation());
			}
		}
	}

	/**
	 * Checks if the notification has been deleted, read or acknowledged, in
	 * which case, this panel should be removed from the pop-up.
	 * <p>
	 * Rules:
	 * <ul>
	 * <li>If deleted, the notification should always be removed</li>
	 * <li>For non-alerts, remove the notification if it is read or
	 * acknowledged.</li>
 ...
 * <p>
 * It will paint a balloon-help style shape around the widget and controls the
 * transparency of the widget using the {@code alpha} field.
 */
class NotificationBorder extends AbstractBorder {
	 ...
	 * 
	 * @param alpha
	 *            the current transparency alpha of the widget
	 */
	 ...
	 *            whether to paint a drop shadow or not
	 */
	...
	public void paintBorder(Component c, Graphics g, int x, int y, int width,
		...
		RoundRectangle2D.Double content = new RoundRectangle2D.Double(pad, pad,
				width - 2 * pad, bottomLineY, cornerRadius, cornerRadius);

 ...
 */
public abstract class NotificationPanel<N extends Notification<?, ?>> extends
	...
	// Standard actions
	protected JButton acknowledgeBtn = new JButton("Acknowledge", EPD.res()
			.getCachedImageIcon("images/notifications/tick.png"));
	protected JButton gotoBtn = new JButton("Goto", EPD.res()
			.getCachedImageIcon("images/notifications/map-pin.png"));
	protected JButton deleteBtn = new JButton("Delete", EPD.res()
			.getCachedImageIcon("images/notifications/cross.png"));
	protected JButton chatBtn = new JButton("Chat", EPD.res()
			.getCachedImageIcon("images/notifications/balloon.png"));
	...
	/**
	 * Returns a reference to the detail panel, which contains the button panel
	 * and the notification detail panel
	 * 
	...
	/**
	 * Returns a reference to the split pane, which contains the table and the
	 * detail panel
	 * 
	...
	/**
	 * Returns the notification with the given identifier. Returns null if none
	 * is found.
	 * 
	...
	/**
	 * Adds the buttons to the button panel. Sub-class can override to customize
	 * the list of buttons to display
	 * 
	 * @param buttonPanel
	 *            the button panel to add the buttons to
	...
	protected ButtonPanel initButtonPanel() {
		...
		buttonPanel.setBorder(BorderFactory.createCompoundBorder(
				BorderFactory.createMatteBorder(0, 0, 2, 0,
						UIManager.getColor("Separator.shadow")),
	...
	/**
	 * This method is called whenever the selection changes to update the
	...
	protected void updateButtonEnabledState() {
		...
		for (N notification : selection) {
			canAcknowledge &= notification.isRead()
					&& !notification.isAcknowledged();
			canDelete &= notification.isAcknowledged();
		}
		acknowledgeBtn.setEnabled(canAcknowledge);
		deleteBtn.setEnabled(canDelete);
		gotoBtn.setEnabled(selection.size() == 1
				&& selection.get(0).getLocation() != null);
	...
	protected void updateChatEnabledState() {
		...
		if (notification != null) {
			chatBtn.setEnabled(notification != null
					&& notification.getTargetId() != null
					&& EPD.getInstance().getChatServiceHandler()
							.availableForChat(notification.getTargetId()));
		} else {
		...
		if (notification != null) {
			if (!notification.isRead()) {
				...
				if (!notification.isAcknowledged()
						&& notification.isAutoAcknowledge()) {
	 ...
	 * 
	 * @param id
	 *            the id of the notification to select
	 */
	...
	/**
	 * If the notification with the given id is the currently selected one, then
	...
	/**
	 * Selects the first unacknowledged row. If none are found, the selection
	 * does not change.
	 */
	...
	/**
	 * This method should be called whenever the notification list should
	 * refresh itself from the back end.
	 * <p>
	...
	/**
	 * Called whenever the notification list should refresh itself from the back
	 * end.
	 * <p>
	 ...
	 * 
	 * @param columnIndex
	 *            the column to set the alignment for
	 * @param alignment
	 *            the {@linkplain JLabel} alignment constant
	 */
	 ...
	 * {@code DefaultTableModel.fireTableDataChanged()} but preserve the
	 * selection
	 */
	...
	/**
	 * Starts a chat session with the target maritime id of the selected
	 * notification
	 */
	protected void chatWithNotificationTarget() {
		...
		if (notification != null
			...
			EPD.getInstance()
					...
					.openNotification(NotificationType.MESSAGES,
							notification.getTargetId(), false);
		}
	}
	...
	protected void notifyListeners() {
		...
		for (N n : tableModel.getNotifications()) {
			...
			if (n.getSeverity() == NotificationSeverity.WARNING) {
				stats.warningCount++;
			}
			if (n.getSeverity() == NotificationSeverity.WARNING
					&& !n.isAcknowledged()) {
				stats.unacknowledgedWarningCount++;
			}
			if (n.getSeverity() == NotificationSeverity.ALERT) {
				stats.alertCount++;
			}
			if (n.getSeverity() == NotificationSeverity.ALERT
					&& !n.isAcknowledged()) {
	...
	/**
	 * Base class used for button panels. Adds an additional button used for
	 * maximizing/minimizing the notification center
	 */
	public static class ButtonPanel extends JPanel {
			...
			addMouseMotionListener(new MouseAdapter() {
				...
				public void mouseMoved(MouseEvent e) {
					if (getOutlineShape().contains(e.getPoint())) {
						setToolTipText(notificationCenter.isMaximized() ? "Minimize Notification Center"
		...
		public Shape getOutlineShape() {
			...
			int s = 20;
			prevWidth = getWidth();
			outlineShape = new RoundRectangle2D.Double(getWidth() - 2 - s, 2,
					s, s, 10, 10);
			return outlineShape;
		}
